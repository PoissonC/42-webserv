...
/*   Updated: 2024/09/09 19:04:20 by ychen2           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "State.hpp"

State::State(int fd, uint32_t client_addr, int socket): isFinishHeaders(false), cgiPID(-1), bodyPos(std::string::npos), contentLength(0), bytes_sent(0), req(std::string()), sock_fd(socket) , conn_fd(fd), stage(&read_request), isCGIrunning(false) {
